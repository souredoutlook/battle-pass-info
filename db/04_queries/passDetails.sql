--TODO: Dry this up
SELECT pass_id as pass, sum(price_estimate) as value, passes.price as price, passes.season as season 
FROM    (SELECT pass_id, pass_items.full_name, concat(pass_items.gear_type, ' ', rarities.full_name) AS pass_items_long_name, COALESCE(price_by_long_name.avg_item_price, price_by_long_name_2.avg_item_price, price_by_long_name_3.avg_item_price, price_by_long_name_4.avg_item_price, price_by_long_name_5.avg_item_price) AS price_estimate
FROM pass_items
...
ON rarities.level + 200 = price_by_long_name_5.level AND pass_items.gear_type = price_by_long_name_5.gear_type) AS query
JOIN passes ON pass_id = passes.id
GROUP BY pass_id, passes.price, passes.season;