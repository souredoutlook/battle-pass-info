SELECT pass_items.full_name, concat(gear_type, ' ', rarities.full_name) AS pass_items_long_name, avg_item_price
FROM pass_items
...
LEFT JOIN (SELECT concat(gear_type, ' ', rarity) as long_name, round(avg(item_price)::numeric, 2) as avg_item_price
                ...
                ON concat(gear_type, ' ', rarities.full_name) = price_by_long_name.long_name
WHERE pass_id = 'Fracture: Tenrai';

-- Interesting query below to sort out the price of an individual item

-- SELECT bundle_items.full_name, concat(gear_type, ' ', rarities.full_name) AS bundle_items_long_name, avg_item_price
-- FROM bundle_items
-- JOIN rarities ON bundle_items.rarity = rarities.level
-- LEFT JOIN (SELECT concat(gear_type, ' ', rarity) as long_name, round(avg(item_price)::numeric, 2) as avg_item_price
--                 FROM    (SELECT bundle_items.full_name, gear_type, rarities.full_name AS rarity, bundle_id, item_price
--                         FROM bundle_items
--                         JOIN    (SELECT bundles.id, price / count as item_price
...
--                 GROUP BY gear_type, rarity) AS price_by_long_name
--                 ON concat(gear_type, ' ', rarities.full_name) = price_by_long_name.long_name
-- WHERE bundle_items.full_name = 'Sundown Smile';
